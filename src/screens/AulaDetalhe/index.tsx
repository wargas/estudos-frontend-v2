...
export default function AulaDetalhe() {
  ...
  const { data: questoesQuery, refetch: refetchQuestao, failureCount } = useQuery<
    Paginate<Questao>
  ...
  const { data: respondidas, refetch: refetchRespondidas } = useQuery<
    Respondida[]
  >(['respondidas', questao?.aula_id, questao?.id], ({ queryKey }) =>
  ...
  async function handlerResponder() {
    ...
    console.log(marcada, questao?.id);
  }
    ...
    <div>
      ...
      <div
        ...
        <div className='flex pt-5 mt-auto'>
          <button
            onClick={() =>
              openDrawer(EditComentario, { id: questao?.id }, refetchComentario)
            }
